...
impl<T: paras_inherent::Config> BenchBuilder<T> {
	...
	// fn create_disputes_with_majority_for
	...
	fn create_disputes_with_only_spam(&self, first_core: u32, last_core: u32) -> Vec<DisputeStatementSet> {
		...
		// 	let ranges_of_validators_needed = num_cores_touched / config.dispute_max_spam_slots;
		// 	// we only have up to 4 ranges of validators possible
		// 	assert!(x <= 4);
		// }
	...
	pub(crate) fn build(self, backed_and_concluding: u32) -> Bench<T> {
		...
		// Note that for the time being we need at least the concluding.
