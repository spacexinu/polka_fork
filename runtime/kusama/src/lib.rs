...
impl pallet_message_lane::Config<PolkadotMessageLaneInstance> for Runtime {
	type Event = Event;
	// TODO: Will need to be updated once Kusama benchmarks are run
	type WeightInfo = ();
...
construct_runtime! {
		...
		UncheckedExtrinsic = UncheckedExtrinsic
	{
		...
		PolkadotBridge: pallet_substrate_bridge::{Module, Call, Storage, Config<T>} = 37,
		PolkadotBridgeCallDispatch: pallet_bridge_call_dispatch::<Instance1>::{Module, Event<T>} = 38,
		PolkadotMessageLane: pallet_message_lane::<Instance1>::{Module, Call, Event<T>} = 39,
	}
}
