...
pub fn run_worker(mem_id: &str, cache_base_path: Option<PathBuf>) -> Result<(), String> {
	let mut memory = match workspace::open(mem_id) {
		Err(e) => {
			debug!(
				target: LOG_TARGET,
	...
	loop {
		...
		let work_item = match memory.wait_for_work(3) {
...
struct ValidationHost {
	...
	memory: Option<workspace::HostHandle>,
	id: u32,
}
...
impl ValidationHost {
	...
	pub fn validate_candidate(
		...
		match memory.request_validation(validation_code, params) {
			Ok(()) => {}
		...
		let result_header = match memory.wait_for_result(EXECUTION_TIMEOUT_SEC) {
